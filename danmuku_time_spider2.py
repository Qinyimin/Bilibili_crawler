...
def getHtmlInfo(url):
    ...
    try:
        ...
        danmuku_url = 'https://comment.bilibili.com/{}.xml'.format(chat_id)
        danmuku_text = requests.get(danmuku_url, headers=headers, timeout=10).text
        time.sleep(5)
        # 匹配弹幕时间（浮点数）
        danmuku_time=[]
        ...
        dict={}
        dict["data"]=subdict_list

        with open('danmuku_time/av'+aid+'_danmu.json', 'a+', encoding='utf-8') as f:
